...
nixos-anywhere --flake .#node0 root@node0.local
nixos-anywhere --flake .#node1 root@node1.local
```
...
### Update single node
```bash
nixos-rebuild switch --flake .#node1 --target-host pi@node1.local --use-remote-sudo
```

### Update all nodes
```bash
for i in {0..3}; do
  nixos-rebuild switch --flake .#node$i --target-host pi@node$i.local --use-remote-sudo
done
```
